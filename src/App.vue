...
<![endif]-->
            ...
            <el-main style="padding:0px;">
                ...
                <el-tabs tab-position="left" v-model="activeName">
                    <el-tab-pane label="使用说明" name="first">
                        ...
                        <p>
                            小学口算试题生成器 V1.0.1 <br/> 作者：高大科技 <br/>
                        </p>
                        <a href="#">点此反馈问题</a>
                    </el-tab-pane>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        共 {{parseInt((res.length + (cols-0) - 1) / (cols-0))}} 行 {{parseInt(((parseInt(res.length + (cols-0) - 1) / (cols-0)) + (pagerows - 1)) / (pagerows - 0))}} 页
                                    </td>
                                </tr>
                                <tr>
                                    <td>尾部补空行：</td>
                                    <td>
                                        <el-switch v-model="appendemptyrows" active-color="#13ce66" inactive-color="#ff4949" border>
                                        </el-switch>
                                    </td>
                                </tr>
                                <tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <el-checkbox style="display:inline-block" v-model="isdiv" label="除" border disabled/>
                                    </td>
                                </tr>
                                <tr>
                                    <td>运算规则：</td>
                                    <td>
                                        <el-select v-model="rule" placeholder="请选择">
                                            <el-option label="已知条件，求得数" value="1">
                                            </el-option>
                                            <el-option label="已知得数，求条件" value="2">
                                            </el-option>
                                        </el-select>
                                        <el-select v-if="'2'==rule" v-model="whichcond" placeholder="请选择">
                                            <el-option label="随机求条件" value="">
                                            </el-option>
                                            <el-option v-for="(rg, i) in range" :key="i" :label="'求条件 '+(i+1)" :value="i">
                                            </el-option>
                                        </el-select>
                                    </td>
                                </tr>
                                    ...
                                    <td>
                                        <el-tooltip class="item" effect="dark" content="注意借/进位设置只对加/减法起作用" placement="bottom-start">
                                            <el-select v-model="borrow" placeholder="请选择">
                                                <el-option label="随机决定" value="random">
                                                </el-option>
                                                <el-option label="不要借/进位" value="no">
                                                </el-option>
                                                <el-option label="全都带借/进位" value="all">
                                                </el-option>
                                            </el-select>
                                        </el-tooltip>
                                    </td>
                                ...
                                <tr v-for="(rg, i) in range" v-bind:key="i">
                                    <td>运算项{{i+1}}取值范围：</td>
                                    <td>
                ...
                <el-alert :closable="false" title="生成结果" type="success" show-icon>
    ...
    <table ref="mycontent" width="100%" border="0" cellpadding="0" cellspacing="0">
        <thead style="display:table-header-group;">
            <tr>
                <th :colspan="cols">
                    <el-popover ref="popover1" placement="top" title="提示" width="280" trigger="hover" content="直接点击页眉区域，可以修改页眉内容。">
                    </el-popover>
        ...
        <tbody>
            <tr v-if="res.length <= 0">
                <td>
                    <el-alert title="还没有生成试题！" type="error" :closable="false">
                    </el-alert>
                </td>
            </tr>
            <tr :class="{break_before:p>1}" v-for="p in parseInt(((parseInt(res.length + (cols-0) - 1) / (cols-0)) + (pagerows - 1)) / (pagerows - 0)) ">
                <td style="border: 0px solid;">
                    <table :style="{fontFamily:fontfamily, fontSize:fontsize+'px'}" width="100%" :cellPadding="cellPadding" :cellSpacing="cellSpacing">
                        <tr v-for="r in (appendemptyrows ? (pagerows - 0) : Math.min(pagerows, parseInt((res.length - (p - 1) * (pagerows-0) * (cols-0) + (cols - 1)) / (cols-0))))" :class="{printonly: r > parseInt((res.length - (p - 1) * (pagerows-0) * (cols-0) + (cols - 1)) / (cols-0))}">
    ...
    <el-dialog title="用户登录" :visible.sync="loginDialogVisible" width="30%" :before-close="handleClose">
    ...
    <el-dialog title="请选择要加载的试卷" :visible.sync="paperDialogVisible" width="30%">
        <el-table height="250" ref="singleTable" :data="tableData" highlight-current-row @current-change="handleCurrentChange">
            <el-table-column type="index" width="50">
            </el-table-column>
            <el-table-column property="date" label="保存时间" width="180">
            </el-table-column>
    ...
    genItem: function() {
      ...
      for (var i = 1; i < this.itemcount; i++, op = this.op()) {
        ...
        if ("*" == op) {
          ...
          // 被乘数为 0? 乘数随机一个值！
          if (r == 0)
            t = randomInt(this.range[1].min, this.range[1].max); // 0 乘以 任何数都等于 0
